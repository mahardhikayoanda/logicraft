...
<body class="h-full">
            ...
            <div class="sidebar-content">
                ...
                <div class="p-4 flex items-center justify-between border-b border-slate-700">
                    <div class="flex items-center">
                        <div
                            class="sidebar-icon w-8 h-8 rounded-lg bg-indigo-600 flex items-center justify-center text-white">
                            <i class="fas fa-cog"></i>
                        </div>
                        <span class="logo-text font-bold text-lg ml-3">Admin Panel</span>
                    </div>
                    <button id="toggle-sidebar" class="text-slate-300 hover:text-white transition-colors">
                ...
                <nav class="flex-1 overflow-y-auto p-2">
                    <a href="{{ route('admin.users.dashboard') }}"
                        class="nav-item text-slate-200 hover:bg-slate-800 rounded-lg
                              {{ request()->routeIs('admin.users.dashboard') ? 'active' : '' }}">
                    ...
                    <a href="{{ route('admin.users.byRole', 'admin') }}"
                        class="nav-item text-slate-200 hover:bg-slate-800 rounded-lg
                              {{ request()->routeIs('admin.users.byRole') && request()->route('role') == 'admin' ? 'active' : '' }}">
                        <div class="sidebar-icon">
                    ...
                    <a href="{{ route('admin.users.byRole', 'owner') }}"
                        class="nav-item text-slate-200 hover:bg-slate-800 rounded-lg
                              {{ request()->routeIs('admin.users.byRole') && request()->route('role') == 'owner' ? 'active' : '' }}">
                        <div class="sidebar-icon">
                            <i class="fas fa-user-tie"></i>
                        </div>
                        <span class="nav-text">Owners</span>
                        <span class="tooltip">Owners</span>
                    </a>

                    <a href="{{ route('admin.users.byRole', 'resepsionis') }}"
                        class="nav-item text-slate-200 hover:bg-slate-800 rounded-lg
                              {{ request()->routeIs('admin.users.byRole') && request()->route('role') == 'resepsionis' ? 'active' : '' }}">
                        <div class="sidebar-icon">
                            <i class="fas fa-concierge-bell"></i>
                        </div>
                        <span class="nav-text">Receptionists</span>
                        <span class="tooltip">Receptionists</span>
                    </a>

                    <a href="{{ route('admin.users.byRole', 'customer') }}"
                        class="nav-item text-slate-200 hover:bg-slate-800 rounded-lg
                              {{ request()->routeIs('admin.users.byRole') && request()->route('role') == 'customer' ? 'active' : '' }}">
                        <div class="sidebar-icon">
                ...
                <div class="p-4 border-t border-slate-700">
                    <div class="flex items-center mb-3">
                        <div
                            class="sidebar-icon w-8 h-8 rounded-full bg-slate-600 flex items-center justify-center text-white">
                            {{ substr(Auth::user()->name, 0, 1) }}
                        </div>
                        <div class="user-info ml-3">
                            <div class="user-name font-medium text-sm">{{ Auth::user()->name }}</div>
                            <div class="user-role text-xs text-slate-400">Admin</div>
                        </div>
                    </div>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit"
                            class="w-full nav-item text-slate-300 hover:bg-slate-800 hover:text-slate-100 rounded-lg text-left">
                            <div class="sidebar-icon">
                ...
                <div class="user-profile-collapsed border-t border-slate-700">
                    <div class="nav-item text-slate-300 hover:bg-slate-800 rounded-lg">
    ...
    <div id="mobile-bottom-menu" class="mobile-bottom-menu md:hidden">
        <div class="px-2 pt-2 pb-4 space-y-1">
            <a href="{{ route('admin.users.dashboard') }}"
                class="mobile-menu-item {{ request()->routeIs('admin.users.dashboard') ? 'active' : '' }}">
                <i class="fas fa-chart-pie"></i>
                <span>Dashboard</span>
            </a>
            <a href="{{ route('admin.users.index') }}"
                class="mobile-menu-item {{ request()->routeIs('admin.users.index') ? 'active' : '' }}">
                <i class="fas fa-users"></i>
                <span>Users</span>
            </a>
            <a href="{{ route('admin.users.byRole', 'admin') }}"
                class="mobile-menu-item {{ request()->routeIs('admin.users.byRole') && request()->route('role') == 'admin' ? 'active' : '' }}">
                <i class="fas fa-user-shield"></i>
                <span>Admins</span>
            </a>
            <a href="{{ route('admin.users.byRole', 'owner') }}"
                class="mobile-menu-item {{ request()->routeIs('admin.users.byRole') && request()->route('role') == 'owner' ? 'active' : '' }}">
                <i class="fas fa-user-tie"></i>
                <span>Owners</span>
            </a>
            <a href="{{ route('admin.users.byRole', 'resepsionis') }}"
                class="mobile-menu-item {{ request()->routeIs('admin.users.byRole') && request()->route('role') == 'resepsionis' ? 'active' : '' }}">
                <i class="fas fa-concierge-bell"></i>
                <span>Receptionists</span>
            </a>
            <a href="{{ route('admin.users.byRole', 'customer') }}"
                class="mobile-menu-item {{ request()->routeIs('admin.users.byRole') && request()->route('role') == 'customer' ? 'active' : '' }}">
                <i class="fas fa-user"></i>
                <span>Customers</span>
            </a>
