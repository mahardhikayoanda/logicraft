                    ...
                    <div class="mt-4 flex flex-wrap gap-2">
                        ...
                        @elseif ($reservation->status === 'canceled')
                            <form action="{{ route('customer.reservations.destroy', $reservation->id) }}" method="POST"
                                ...
                                @csrf @method('DELETE')
                                <button type="submit"
                                    class="bg-gray-700 hover:bg-gray-800 text-white px-3 py-1 rounded text-sm">Hapus Reservasi</button>
                            </form>
                        @elseif ($reservation->status === 'confirmed')
                            <a href="{{ route('customer.reservations.show', $reservation->id) }}"
                                class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-sm">Lihat Detail</a>

                            @if ($reservation->review)
                                {{-- Tombol edit ulasan --}}
                                <button onclick="openReviewModal({{ $reservation->review->id }}, {{ $reservation->review->rating }}, `{{ $reservation->review->comment }}`)"
                                    class="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded text-sm">Edit Ulasan</button>
                            @else
